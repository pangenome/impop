...
Optional arguments:
  ...
  -m  Method: direct or grouped (default: direct)
  -t  Threshold for grouping (default: 0.999, only used with -m grouped)
...
Methods:
  - direct: Calculate diversity as average of pairwise differences
  - grouped: Group sequences with similarity > threshold, then use frequency-based formula
USAGE
    exit 1
}
...
process_region() {
    ...
    local tmp_sim
    tmp_sim=$(mktemp "tmp.similarities.XXXXXX")
    ...
    local fst_output
    if ! fst_output=$("${fst_cmd[@]}" 2>&1); then
...
while getopts "A:B:b:p:s:r:m:t:o:d:P:vh" opt; do
    case $opt in
        A) POP_A_FILE="$OPTARG" ;;
        B) POP_B_FILE="$OPTARG" ;;
        b) BED_FILE="$OPTARG" ;;
